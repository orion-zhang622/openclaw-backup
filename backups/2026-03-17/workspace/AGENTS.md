...
**何时触发：** 主人给出一个目标/任务时，默认按此流程执行。

### Step 1: 目标解析 (5W1H)
```
What: 具体要做什么？
Why: 背后的真实需求是什么？
Constraint: 时间、预算、技术栈限制？
Deliverable: 最终交付物是什么格式？
```

### Step 2: 能力盘点
```
1. memory_recall("相关关键词") → 搜索已有经验
2. 检查本地技能目录 → ~/.openclaw/skills/
3. 需要新技能？ → clawhub search / find-skills
4. 在线搜索 → 按工具优先级执行
```
...
| 场景 | 首选 | 备选 |
|------|------|------|
| 全网搜索 | tavily | baidu-search → kimi_search |
| 技术文档 | agent-reach GitHub | web_fetch |
| 社交平台 | agent-reach 对应平台 | 浏览器自动化 |
| 代码实现 | 现有 skill | GitHub 搜索 → 自研 |

### Step 4: 方案设计
```
1. 搜索"有没有人做过" → GitHub / 社区 / 小红书
2. 评估"适不适合我" → 复杂度 vs 时间约束
3. 确定"能不能做得更好" → 定制化改进点
```
...
小步快跑：先跑通最小可行版本 (MVP)
边做边验证：不要假设，测试为准
记录踩坑：同步写入 memory-lancedb-pro
```

### Step 6: 交付 + 归档
```
交付物：按 Rule 6 上传到飞书 openclaw 文件夹
复盘：关键决策 → MEMORY.md
踩坑：失败教训 → .learnings/
```

### 核心原则
1. **先搜后造** — 80%的需求别人已经解决过
2. **技能优先** — 能用现成工具绝不重复造轮子
3. **渐进交付** — 先给能用的，再给完美的
4. **记忆写入** — 每个踩坑都要变成可检索的知识
